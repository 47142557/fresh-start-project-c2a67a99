...
const FormQuote = () => {
  ...
  const [aportesType, setAportesType] = useState(null); // 'D' or 'P'
  ...
  const updatePersonalData = useCallback((field, value) => {
    setFormData(prev => ({
      ...prev,
      personalData: {
        ...prev.personalData,
  ...
  const updateChildAge = useCallback((index, value) => {
  ...
  const incrementar = (member) => {
    if (member === 'titular') {
      setEdadTitular(prev => prev + 1);
    } else if (member === 'conyuge' && selectedGroup > 2) {
  ...
  const decrementar = (member) => {
    if (member === 'titular' && edadTitular > 18) {
      setEdadTitular(prev => prev - 1);
    } else if (member === 'conyuge' && edadConyuge > 0 && selectedGroup > 2) {
      setEdadConyuge(prev => prev - 1);
  ...
  const incrementarChildAge = (index) => {
    const currentAge = formData[`edadHijo${index + 1}`] || 0;
    updateChildAge(index, currentAge + 1);
  };

  const decrementarChildAge = (index) => {
    const currentAge = formData[`edadHijo${index + 1}`] || 0;
    if (currentAge > 0) {
      updateChildAge(index, currentAge - 1);
    }
              ...
              {!cotizacionVisible ? (
                ...
                !contactoType ? (
                  ...
                  <div className="thankyou-card text-center p-8 bg-green-50 rounded-xl shadow-lg">
                    <img className="icon icon--large mx-auto mb-4" src={contactoType === 'phone' ? `https://placehold.co/60x60/${PRIMARY_COLOR.substring(1)}/ffffff?text=CALL` : 'https://placehold.co/60x60/25D366/ffffff?text=WA'} alt="ícone de agradecimento" />
